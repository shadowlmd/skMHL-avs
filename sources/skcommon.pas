...
===============================================================================
 ...
 skCommon.pas - part of skMHL Library by sergey korowkin (2:6033/27@fidonet),
                port to *nix based OS by Andrew V. Sichevoi (2:6028/9@fidonet)

 Common skMHL Library unit.
...
procedure UnixDateTimeToMessageBaseDateTime(SecsPast: Int64; var DateTime: TMessageBaseDateTime);
procedure MessageBaseDateTimeToUnixDateTime(const DateTime: TMessageBaseDateTime; var SecsPast: Int64);
...
procedure Delay(const ms: Longint);
...
function CreateDirectoryA(Directory: String): Boolean;
 begin
  ...
  if (Length(Directory) <> 3) and (Directory[Length(Directory)] = Slash) then
...
function CreateDirectory(Directory: String): Boolean;
 ...
 begin
  ...
  if Directory[Length(Directory)] = Slash then
   Delete(Directory, Length(Directory), 1);

  S:='';

  for K:=1 to Length(Directory) do
   if Directory[K] = Slash then
    begin
     CreateDirectoryA(S);
