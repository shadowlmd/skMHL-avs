...
===============================================================================
 ...
 skMHLmsg.pas - part of skMHL Library by sergey korowkin (2:6033/27@fidonet),
                port to *nix based OS by Andrew V. Sichevoi (2:6028/9@fidonet)

 Unit for work with Opus message base
...
function TFidoMessageBase.OpenMessage: Boolean;
 begin
  ...
  Link:=CreateMessageBaseFileStream(GetBasePath + LongToStr(Current) + ExtMSG, smOpen);
...
function TFidoMessageBase.CreateNewMessage: Boolean;
 begin
  ...
  Link:=CreateMessageBaseFileStream(GetBasePath + LongToStr(Current) + ExtMSG, smCreate);
...
function TFidoMessageBase.KillMessage: Boolean;
 begin
  ...
  if not KillFile(GetBasePath + LongToStr(Current) + ExtMSG) then
   Exit;
...
{$ENDIF}
 begin
  ...
  if Find^.StartSearch(GetBasePath + '*'+ExtMSG, faReadonly + faArchive) then
   repeat
