...
===============================================================================
 ...
 Unit for work with Squish message base
...
function TSquishMessageBase.Open(const Path: String): Boolean;
 begin
  ...
  if not FileExists(GetBasePath + esSQD) then
   begin
  ...
  if not FileExists(GetBasePath + esSQI) then
   begin
...
function TSquishMessageBase.Exist(const Path: String): Boolean;
 begin
  ...
  Exist:=FileExists(GetBasePath + esSQD) and FileExists(GetBasePath + esSQI);
 end;
...
function TSquishMessageBase.Exists(Message: Longint): Boolean;
 var
  ...
  IndexPos: Longint;
 begin
  Exists:=CheckIndex(Message, Index, IndexPos, False);
 end;

procedure TSquishMessageBase.Seek(Message: Longint);
 begin
  if CheckIndex(Message, SquishIndex, SquishIndexPos, True) then
   SetCurrent(SquishIndex.Number)
  else
   begin
    SetCurrent(0);
    SquishIndexPos:=0;
...
function TSquishMessageBase.CreateNewMessage: Boolean;
 begin
  ...
  SetCurrent(SquishBaseHeader.UID);

  SquishMessageHeader.UID:=Current;
