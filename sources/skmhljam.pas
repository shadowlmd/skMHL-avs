...
===============================================================================
 ...
 skMHLjam.pas - part of skMHL Library by sergey korowkin (2:6033/27@fidonet),
                port to *nix based OS by Andrew V. Sichevoi (2:6028/9@fidonet)

 Unit for work with JAM message base
...
function TJamMessageBase.Open(const Path: String): Boolean;
 ...
 begin
  ...
  if not FileExists(GetBasePath + ejJHR) then
   begin
  ...
  if not FileExists(GetBasePath + ejJDX) then
   begin
  ...
  if not FileExists(GetBasePath + ejJDT) then
   begin
  ...
  repeat
   HeaderLink:=CreateMessageBaseFileStream(GetBasePath + ejJHR, smOpen or smDenyWrite);
   ...
   IndexLink:=CreateMessageBaseFileStream(GetBasePath + ejJDX, smOpen or smDenyWrite);
   ...
   DataLink:=CreateMessageBaseFileStream(GetBasePath + ejJDT, smOpen or smDenyWrite);
   ...
   LastReadLink:=CreateMessageBaseFileStream(GetBasePath + ejJLR, smOpen);

   if LastReadLink^.Status <> smOk then
    begin
     Dispose(LastReadLink, Done);

     LastReadLink:=CreateMessageBaseFileStream(GetBasePath + ejJLR, smCreate);
...
function TJamMessageBase.Create(const Path: String): Boolean;
 ...
 begin
  ...
  repeat
   HeaderLink:=CreateMessageBaseFileStream(GetBasePath + ejJHR, smCreate or smDenyWrite);
   ...
   IndexLink:=CreateMessageBaseFileStream(GetBasePath + ejJDX, smCreate or smDenyWrite);
   ...
   DataLink:=CreateMessageBaseFileStream(GetBasePath + ejJDT, smCreate or smDenyWrite);
   ...
   LastReadLink:=CreateMessageBaseFileStream(GetBasePath + ejJLR, smCreate or smDenyWrite);
   ...
   MessageBaseDateTimeToUnixDateTime(DateTime, I);
   JamBaseHeader.Created:=Longword(I);
...
function TJamMessageBase.Exist(const Path: String): Boolean;
 ...
 begin
  ...
  Exist:=FileExists(APath + ejJHR) and
         FileExists(APath + ejJDX) and
         FileExists(APath + ejJDT) and
         FileExists(APath + ejJLR);
 end;
...
procedure TJamMessageBase.SetWrittenDateTime(var DateTime: TMessageBaseDateTime);
 ...
 begin
  MessageBaseDateTimeToUnixDateTime(DateTime, I);
  JamMessageHeader.JamHeader.DateWritten:=Longword(I);
 end;
...
procedure TJamMessageBase.SetArrivedDateTime(var DateTime: TMessageBaseDateTime);
 ...
 begin
  MessageBaseDateTimeToUnixDateTime(DateTime, I);
  JamMessageHeader.JamHeader.DateArrived:=Longword(I);
 end;
